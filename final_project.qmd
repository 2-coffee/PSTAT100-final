...
The variables that are most influential are `edu_bach`, `edu_grad`, `ed_less_hs`, `total_mort`, and `hc_private`. States with high PC1 scores indicate a relatively higher than average proportion of residents with private insurance, bachelors degree, and graduate degrees. While states with a lower PC1 value indicates a relatively higher than average proportion of residents who have less than a high school education and higher infant mortality rate.

[Name for PC2:]{.underline} *"Education and Income"*

The variables that are most influential for PC2 are `edu_somecoll`, `income_median`, `SNAP`, `total_fer`, and `edu_grad`. States with a high PC2 value indicate higher proportions of residents who only attained some college education and has higher TFR, on the other hand, states with low PC2 values has a higher proportion graduate degrees, higher income median, and SNAP rate.

[Name for PC3:]{.underline} *"Public/None Healthcare Coverage and High School Educated Population"*

The variables that are most influential for PC3 are `hc_public`, `edu_hs`, `income_median`, and `hc_nohc`. Lower PC3 scores are associated with states that have greater public healthcare coverage and a larger proportion of the population with only a high school diploma. Conversely, high PC3 scores are linked to states with higher median incomes and a greater percentage of people without health insurance coverage.

[Name for PC4:]{.underline} *"Private/None Healthcare Coverage and SNAP"*

The variables that are most influential for PC4 are `hc_nohc`, `SNAP`, `hc_private`, `edu_hs`, and `total_mort`. High PC4 scores explain states who has higher SNAP rates, proportion of private healthcare coverage, proportion of only high school diploma, and has higher infant mortality. Whereas a low PC4 indicates relatively higher proportion of citizens who has no healthcare coverage.
...
The variables that are most influential for PC5 are `edu_somecoll`, `edu_hs`, `total_fer`, and `edu_grad`. High PC5 indicates states with higher proportion of residents who only attained some college education. Lower PC5 scores indicate states who has a relatively higher proportion of high school graduates, graduate degrees, and higher TFR.
