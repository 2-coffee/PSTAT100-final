...
hc_gap <- main_pop4 |> select(state,year,starts_with("hc_")) |> # grouping public and noHC to one column because of PC
  mutate(
    hc_PubNo = hc_public + hc_nohc,
    hc_gap = hc_private - hc_PubNo
  )
...
-   problems
    ...
    Fertility and infant mortality rate places a huge role in population growth and indicating a country's overall health, reflecting the social, economic, and healthcare conditions. As we proceed into the future, we are questioning how major conflicts are going to impact our population. Through this project, we went through different methods to explore fertility and infant mortality rates across U.S. states. We identified the different trends within ethnicities for U.S. regions and found Non-Hispanic White populations had the highest variation in infant mortality among the other ethnic groups. In addition, age groups, marital status, economic indicators, and health coverage are correlated with total fertility and infant mortality rates. We identified education attainment and healthcare coverage as the most significant patterns using PCA and tested their significance with linear regression for both TFR and IMR. Using linear regression, we saw the change of estimates of significant predictiors across years. Seeing the importance of educational attainment, we examined the education gaps with the highest and lowest differences in higher education (graduate and above) and lower education (bachelors and below). We used the table to examine other trends in the different categories of healthcare coverage. (Add Averages Per Region)
        Throughout our analysis, we encountered several instances where data for ethnicities, other than Non-Hispanic White, were either omitted or replaced with zeroes. This posed significant challenges in our analysis, as our techniques indicated no statistical significance or produced missing values (NAs). In the future, we would like to obtain and filter data by counties and ethnicities to better assess maternal and infant health disparities in the United States.
