...
fertility_merged2 <- fertility_merged %>% 
  ...
  mutate(prop_births_1524 = births_age1524 / births_total,
         ...
         fertility_rate_state = 9 * (fer_rate_1524 + fer_rate_2534 + fer_rate_3544)
         )
```
...
births_deaths <- fertility_mortality %>% 
  select(state, year, bacode, births_age1524:births_nonmedicaid, fer_rate_1524:fertility_rate_state, exposed_infdeaths, deaths_nhblack:deaths_total, infant_births_total:infantMort_deaths_total, fer_rate_nhblack:fer_rate_otherraceeth ) %>% 
...
births_deaths %>% 
  ...
  summarize(total_births_us = sum(norm_total_births, na.rm = TRUE),
            tfr = sum(fertility_rate_state * total_births_us, na.rm = TRUE)/ sum(total_births_us, na.rm = TRUE)) %>% 
  ungroup()
```
...
pop_year <- fertility_mortality |> select(year,matches("pop_age[0-9]")) |> unique() |>
  aggregate(. ~ year, FUN=sum)

total_fertilityUS <- fertility_mortality |> select(year,matches("births_age[0-9]")) |>
  aggregate(. ~ year, FUN=sum) |>
  left_join(pop_year) |>
