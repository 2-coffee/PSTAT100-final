...
## Normalize each variable to tota population 
...
#run this for linear regression
main_popRegion4 <- main_popRegion |>
  mutate(
  across(matches("total_[a-z]+_pop"), ~ .x/total_pop),
  across(matches("pop_age[0-9]"), ~ .x/total_pop),
  across(matches("ms_[a-z]"), ~ .x/total_pop),
  ...
  left_join(TFR_region1) |>
select(region, state, year, starts_with("edu_"), income_median, SNAP, starts_with("hc"), -edu_attain_pop, total_fer) |>
  mutate(
    edu_hsORless = edu_hs + edu_less_hs,
    edu_bachORmore = edu_bach + edu_grad,
  )
```
...
lin_al <- lm(total_fer ~ edu_less_hs + edu_hs + edu_somecoll + edu_bach +edu_grad + income_median + hc_private + hc_public + hc_nohc, data = lr_al)
summary(lin_al)
```
...
```{r}
print("2016")
lin_reg16 <- lm(total_fer ~ edu_less_hs + edu_hs + edu_somecoll + edu_bach +edu_grad + income_median+ SNAP + hc_private + hc_public + hc_nohc + total_mort, data = lr_state16)
summary(lin_reg16)

print("2017")
lin_reg17 <- lm(total_fer ~ edu_less_hs + edu_hs + edu_somecoll + edu_bach +edu_grad + income_median+ SNAP + hc_private + hc_public + hc_nohc + total_mort, data = lr_state17)
summary(lin_reg17)

print("2018")
lin_reg18 <- lm(total_fer ~ edu_less_hs + edu_hs + edu_somecoll + edu_bach +edu_grad + income_median+ SNAP + hc_private + hc_public + hc_nohc + total_mort, data = lr_state18)
summary(lin_reg18)

print("2019")
lin_reg19 <- lm(total_fer ~ edu_less_hs + edu_hs + edu_somecoll + edu_bach +edu_grad + income_median+ SNAP + hc_private + hc_public + hc_nohc + total_mort, data = lr_state19)
summary(lin_reg19)

print("2021")
lin_reg21 <- lm(total_fer ~ edu_less_hs + edu_hs + edu_somecoll + edu_bach +edu_grad + income_median+ SNAP + hc_private + hc_public + hc_nohc + total_mort, data = lr_state21)
summary(lin_reg21)

print("2022")
lin_reg22 <- lm(total_fer ~ edu_less_hs + edu_hs + edu_somecoll + edu_bach +edu_grad + income_median+ SNAP + hc_private + hc_public + hc_nohc + total_mort, data = lr_state22)
summary(lin_reg22)

print("2023")
lin_reg23 <- lm(total_fer ~ edu_less_hs + edu_hs + edu_somecoll + edu_bach +edu_grad + income_median+ SNAP + hc_private + hc_public + hc_nohc + total_mort, data = lr_state23)
summary(lin_reg23)
...
### linear regression per region by year
```{r}
midw_reg <- main_popRegion4 |>
...
model <- lm(total_fer ~ edu_hsORless + edu_somecoll + edu_bachORmore + 
                SNAP + hc_private + hc_public + hc_nohc, data = midw_reg16)
summary(model)
...
```{r}
noeast_reg <- main_popRegion |>
  filter(region == "Northeast")

noeast_reg16 <- noeast_reg |>
  filter(year == 2016)
noeast_reg17 <- noeast_reg |>
  filter(year == 2017)
noeast_reg18 <- noeast_reg |>
  filter(year == 2018)
noeast_reg19 <- noeast_reg |>
  filter(year == 2019)
noeast_reg21 <- noeast_reg |>
  filter(year == 2021)
noeast_reg22 <- noeast_reg |>
  filter(year == 2022)
noeast_reg23 <- noeast_reg |>
  filter(year == 2023)

soeast_reg <- main_popRegion |>
  filter(region == "Southeast")

soeast_reg16 <- soeast_reg |>
  filter(year == 2016)
soeast_reg17 <- soeast_reg |>
  filter(year == 2017)
soeast_reg18 <- soeast_reg |>
  filter(year == 2018)
soeast_reg19 <- soeast_reg |>
  filter(year == 2019)
soeast_reg21 <- soeast_reg |>
  filter(year == 2021)
soeast_reg22 <- soeast_reg |>
  filter(year == 2022)
soeast_reg23 <- soeast_reg |>
  filter(year == 2023)

sowest_reg <- main_popRegion |>
  filter(region == "Southeast")

sowest_reg16 <- sowest_reg |>
  filter(year == 2016)
sowest_reg17 <- sowest_reg |>
  filter(year == 2017)
sowest_reg18 <- sowest_reg |>
  filter(year == 2018)
sowest_reg19 <- sowest_reg |>
  filter(year == 2019)
sowest_reg21 <- sowest_reg |>
  filter(year == 2021)
sowest_reg22 <- sowest_reg |>
  filter(year == 2022)
sowest_reg23 <- sowest_reg |>
  filter(year == 2023)

west_reg <- main_popRegion |>
  filter(region == "West")
...
run_regression <- function(data, region, year) {
  # Filter data by region and year
  region_year_data <- data |> filter(region == !!region, year == !!year)
  
  # Run the linear regression
  model <- lm(total_fer ~ edu_less_hs + edu_hs + edu_somecoll + edu_bach + edu_grad + 
                SNAP + hc_private + hc_public + hc_nohc, data = region_year_data)
  
  return(model)
}

midwest_regression_2016 <- run_regression(main_popRegion4, "Midwest", 2016)

# Get summary of the regression model
summary(midwest_regression_2016)
```
