...
prop_only_merge <- infant_mort1 |>
  ...
  mutate(
    norm_births_nhblack = (prop_births_nhblack.x + prop_births_nhwhite.y)/2,
  ...
  mutate(across(starts_with("deaths_"), ~ .x/norm_total_births*1000, .names="infantMort_{.col}")) |>
  mutate(across(starts_with("norm_births_"), ~ .x/norm_total_births*1000, .names="fertilityRate_{.col}"))
