...
maternal_mort <- read_csv("~/Maternal_Death_Counts.csv")
```
```{r}
...
maternal_byRace <- maternal_mort1 |> filter(!is.na(`Race and Hispanic origin`)) |>
  rename("RaceEth"=`Race and Hispanic origin`)

head(maternal_byAge)
head(maternal_byRace)
okabe <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#D55E00", "#CC79A7")

maternal_byRace |> ggplot(aes(x=`Year of Death`, y=`Maternal Mortality Rate`, color=RaceEth)) +
  geom_point()
maternal_byRace |> ggplot(aes(x=`Live Births`, y=`Maternal Deaths`, color=as.factor(`Year of Death`))) +
  geom_point() + 
